...
export default function AdminOrders() {
  const [orders, setOrders] = useState<Order[]>([
    { id: '1', customerName: 'Jean Dupont', total: 199.99, status: 'En cours', date: '2024-06-23' },
    { id: '2', customerName: 'Marie Martin', total: 349.99, status: 'En attente', date: '2024-06-22' },
    { id: '3', customerName: 'Pierre Durand', total: 89.99, status: 'Livré', date: '2024-06-21' }
  ]);
  
  const [showForm, setShowForm] = useState(false);
  const [editingOrder, setEditingOrder] = useState<Order | null>(null);
  const [formData, setFormData] = useState({ customerName: '', total: '', status: 'En attente' as Order['status'] });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingOrder) {
      ...
      setOrders(orders.map(o => 
        o.id === editingOrder.id 
          ? { ...o, customerName: formData.customerName, total: parseFloat(formData.total), status: formData.status }
          : o
      ));
    } else {
      // Add new order
    ...
    setFormData({ customerName: '', total: '', status: 'En attente' });
  };

  const handleEdit = (order: Order) => {
    setEditingOrder(order);
    setFormData({ customerName: order.customerName, total: order.total.toString(), status: order.status });
    setShowForm(true);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-3xl font-bold text-cyan-400">Gestion des Commandes</h2>
        <button 
      ...
      {showForm && (
        <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
          <h3 className="text-xl font-bold mb-4 text-cyan-400">
            {editingOrder ? 'Modifier la commande' : 'Ajouter une commande'}
          </h3>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-gray-300 mb-1">Nom du client</label>
              <input
                type="text"
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-gray-800 rounded-lg p-6 border border-gray-700 max-w-md w-full mx-4">
            ...
            <p className="text-gray-300 mb-6">Êtes-vous sûr de vouloir supprimer cette commande ?</p>
            <div className="flex gap-4">
              <button 
